  ...
  return (
    <div style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif", color: "#0f0f0f", background: "#fff", minHeight: "100vh" }}>
      <style>{`
        ...
        .lp-feat-card {
          background: #faf8ff; border: 1px solid #ede9ff; border-radius: 14px;
          padding: 24px 22px;
        }
        .lp-cta-section {
          background: linear-gradient(135deg, #7c3aed 0%, #5b21b6 100%);
          border-radius: 20px; padding: 64px 40px; text-align: center; color: #fff;
        }
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            {user ? (
              ...
              <>
                ...
                <button
                  onClick={onStart}
                  style={{ background: "#0f0f0f", color: "#fff", border: "none", borderRadius: "8px", padding: "8px 18px", fontSize: "13px", fontWeight: "700", cursor: "pointer" }}
                >
                  Try Free
                </button>
              </>
      ...
      <div className="lp-hero-pad" style={{ maxWidth: "1000px", margin: "0 auto", padding: "88px 24px 64px", textAlign: "center" }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: "6px", background: "#f0eaff", border: "1px solid #ddd6fe", borderRadius: "20px", padding: "5px 14px", marginBottom: "28px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "12px" }}>
          ...
          <span style={{ fontSize: "12px", color: "#aaa" }}>No account needed · 10 free tailors per day</span>
        </div>

        {/* Stats bar */}
        <div style={{ display: "flex", justifyContent: "center", gap: "40px", marginTop: "64px", paddingTop: "40px", borderTop: "1px solid #f0f0f0", flexWrap: "wrap" }}>
          {[
      ...
      {/* How it works */}
      <div className="lp-section-pad" style={{ padding: "72px 24px", background: "#fafafa" }}>
        <div style={{ maxWidth: "1000px", margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: "48px" }}>
            <div style={{ fontSize: "12px", fontWeight: "700", color: "#7c3aed", letterSpacing: "1.5px", textTransform: "uppercase", marginBottom: "10px" }}>How it works</div>
          ...
          <div className="lp-steps-grid" style={{ display: "flex", gap: "16px" }}>
            {[
              { n: "1", icon: "📄", title: "Paste Your Resume", desc: "Upload a PDF or DOCX, or paste your resume text directly. Any format works." },
              { n: "2", icon: "📋", title: "Add the Job Description", desc: "Paste the full job posting. The more detail, the better the match." },
              { n: "3", icon: "✅", title: "Download Your Resume", desc: "Get a rewritten resume with a match score, ATS keywords, and skill gaps identified." },
            ].map(({ n, icon, title, desc }) => (
              <div key={n} className="lp-step-card">
                <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "14px" }}>
                  <div style={{ width: "32px", height: "32px", background: "#7c3aed", color: "#fff", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "13px", fontWeight: "800", flexShrink: 0 }}>{n}</div>
      ...
      {/* Features */}
      <div className="lp-section-pad" style={{ padding: "72px 24px", background: "#fff" }}>
        <div style={{ maxWidth: "1000px", margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: "48px" }}>
            <div style={{ fontSize: "12px", fontWeight: "700", color: "#7c3aed", letterSpacing: "1.5px", textTransform: "uppercase", marginBottom: "10px" }}>Why ResumeJD</div>
          ...
          <div className="lp-feat-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
            {[
              { icon: "🎯", title: "Match Score", desc: "Get a 0–100 score showing exactly how well your background fits the role. Know before you apply." },
              { icon: "🚫", title: "No Fabrication", desc: "We never invent jobs, fake credentials, or add experience you don't have. Only your real background, reframed." },
              { icon: "🤖", title: "ATS Optimized", desc: "Every bullet is rewritten using the job description's exact keywords — so you pass the automated screening." },
              { icon: "🧠", title: "Learns Your Background", desc: "Sign in and the AI remembers every resume you've uploaded — getting better the more you use it." },
              { icon: "📊", title: "Gap Analysis", desc: "See exactly which requirements you cover, which you partially bridge, and which are genuine gaps to address." },
              { icon: "📥", title: "PDF & DOCX Export", desc: "Download your tailored resume as a professionally formatted PDF or Word document, ready to submit." },
            ].map(({ icon, title, desc }) => (
              <div key={title} className="lp-feat-card">
                <div style={{ fontSize: "26px", marginBottom: "12px" }}>{icon}</div>
                <div style={{ fontSize: "14px", fontWeight: "700", marginBottom: "7px", color: "#0f0f0f" }}>{title}</div>
                <div style={{ fontSize: "13px", color: "#666", lineHeight: "1.65" }}>{desc}</div>
              </div>
            ))}
          </div>
      ...
      <div className="lp-section-pad" style={{ padding: "72px 24px", background: "#fafafa" }}>
        <div style={{ maxWidth: "700px", margin: "0 auto" }}>
          <div className="lp-cta-section">
            <div style={{ fontSize: "13px", fontWeight: "600", color: "#c4b5fd", letterSpacing: "1px", textTransform: "uppercase", marginBottom: "16px" }}>Get started free</div>
            ...
            <p style={{ fontSize: "16px", color: "#c4b5fd", marginBottom: "36px", lineHeight: "1.6" }}>
              Every job is different. Your resume should be too.
            </p>
            <button
              className="lp-btn-primary"
              onClick={onStart}
              style={{ background: "#fff", color: "#7c3aed", fontSize: "16px", padding: "15px 36px" }}
            >
              Tailor My Resume Now
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
            </button>
            <div style={{ marginTop: "16px", fontSize: "12px", color: "#a78bfa" }}>Free · 10 tailors per day · No signup required</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ borderTop: "1px solid #f0f0f0", padding: "24px", textAlign: "center" }}>
        <div style={{ maxWidth: "1000px", margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "8px" }}>
          <span style={{ fontWeight: "800", fontSize: "15px", letterSpacing: "-0.3px" }}>ResumeJD</span>
          <span style={{ fontSize: "12px", color: "#aaa" }}>© 2026 ResumeJD. Free AI resume tailor.</span>
        </div>
      </div>
    </div>
  );
